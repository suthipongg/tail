...
'''
Reach5Mini Requester Class (for the RS485 bus) - ability to subscribe and publish requested data to the Reach5Mini passthrough scripts
Arguents:
...
'''
class Reach5MiniROSRequesterRS485():
	instances = []
		...
		while not rospy.is_shutdown():	
			if self.device_ids:		
				for device_id in self.device_ids: 
					...
					'''
					Wait for request packets to return before sending new packets.
					Since the RS485 bus is half-duplex dual-way communication system (has no separate transmit and receive lines), we must not send data at the same time as receiving.
					If data was to be transmitted at the same time as receiving, there could be bit conflicts on the line which will corrupt data.
					'''			
...
if __name__ == "__main__":
    ...
    # Create Reach5MiniROSRequesterRS485 instance (with instance number '0')
    r5m_0 = Reach5MiniROSRequesterRS485(r5m_instance=instance)
    r5m_0.start()
